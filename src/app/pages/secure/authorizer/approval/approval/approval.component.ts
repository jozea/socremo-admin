...
export class ApprovalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //   representativesInfo: this.formBuilder.array([ this.createItem() ])
    // })

    // console.log(this.data)
    this.arr = this.data.details.representativesInfo
    this.corporateInfo = this.data.details
    this.isApproved = this.data.approved
  ...
  approveRepresentative() {
    this.isLoading = true
    let param = {
      _id: this.corporateInfo._id
    }
    // console.log(param)
    this.corporateService.approveOnboardedReps(param).then((response: any)=> {
      // console.log(response)
      if (response.status == true) {
        this.utilService.triggerNotification(response.message);
        this.isLoading = false
        this.router.navigate(["/app/onboard"]);
      }else {
        this.utilService.triggerNotification(response.message);
        this.isLoading = false
      }
